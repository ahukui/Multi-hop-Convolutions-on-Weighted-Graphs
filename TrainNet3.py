...
def MineModel(feature, support, ACT, weight_deacy, G, G1, G2):
    ...
    out = GraphFlusing('attention',activation='softmax')([Y0,Y1,Y1,Y2])#Activation('softmax')(add([Y0,Y1]))#
    
    # Compile model
    model = Model(inputs=[X_in1]+G+G1+G2, outputs=out)
...
def Training():
    ...
    for fold in range(8,9):#num_folds
        print('Starting fold {}'.format(fold + 1))
          
        model = MineModel(feature, support, ACT, weight_deacy, G, G1, G2)
        # Create model
...
#        train_results = []
    ...
    #        # Training step
        ...
        model.save('Mixed/Mix3_best_model_%d.h5'%(fold))
...
def test():
    ...
    for fold in range(num_folds):#num_folds
        ...
        model.load_weights('Mixed/Mix3_best_model_%d.h5'%(fold))
        preds = model.predict(graph0, batch_size=557)        
        test_loss, test_acc = evaluate_preds(preds, [y_test], [init_test_mask])
        Final_Score.append(test_acc[0]) 
    print(Final_Score)
