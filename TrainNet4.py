...
def MineModel(feature, support, ACT, weight_deacy, G, G1, G2, G3):
    ...
    H2 = GraphConvolution(16, support, activation=ACT, kernel_regularizer=l2(weight_deacy))([H]+G2)
    H2 = Dropout(0.3)(H2)
    Y2 = GraphConvolution(3, support, activation=ACT)([H2]+G2)

    H3 = GraphConvolution(16, support, activation=ACT, kernel_regularizer=l2(weight_deacy))([H]+G3)
    H3 = Dropout(0.3)(H3)
    Y3 = GraphConvolution(3, support, activation=ACT)([H3]+G3)


    out = GraphFlusing('attention',activation='softmax')([Y0,Y1,Y1,Y2,Y3])#Activation('softmax')(add([Y0,Y1]))#
    
    # Compile model
    model = Model(inputs=[X_in1]+G+G1+G2+G3, outputs=out)
...
def Training():
    ...
    adj3 = np.load('W_G3_3.npy')
    adj3 = sp.csr_matrix(adj3/9., dtype=np.float32)
    adj4 = np.load('W_G4_4.npy')
    adj4 = sp.csr_matrix(adj4/16., dtype=np.float32)
          
    print('Using Chebyshev polynomial basis filters...')
    T_k0 = Compute_Matrix(adj1) 
    T_k1 = Compute_Matrix(adj2) 
    T_k2 = Compute_Matrix(adj3)     
    T_k3 = Compute_Matrix(adj4)        
    graph0 = [feature]+T_k0+T_k1+T_k2+T_k3
    support = MAX_DEGREE + 1
    G = [Input(shape=(None, None), batch_shape=(None, None), sparse=True) for _ in range(support)]
    G1 = [Input(shape=(None, None), batch_shape=(None, None), sparse=True) for _ in range(support)]
    G2 = [Input(shape=(None, None), batch_shape=(None, None), sparse=True) for _ in range(support)] 
    G3 = [Input(shape=(None, None), batch_shape=(None, None), sparse=True) for _ in range(support)]     
    ...
    for fold in range(1):#num_folds
        ...
        model = MineModel(feature, support, ACT, weight_deacy, G, G1, G2, G3)
        # Create model
...
#        train_results = []
    ...
    #        # Training step
        ...
        model.save('Mixed/Mix4_best_model_%d.h5'%(fold))
...
def test():
   # Loading data
    ...
    for fold in range(num_folds):#num_folds
        ...
        init_val_mask = val_mask
        init_test_mask = test_mask
        print(np.sum(init_train_mask)+np.sum(init_val_mask)+np.sum(init_test_mask))
        ...
        model.load_weights('Mixhop_%d.h5'%(fold))
        preds = model.predict(graph0, batch_size=557)      
        
        labels, indices = [y_test], [init_test_mask]
        PreLabel = np.zeros_like(init_test_mask)
        for i in range(init_test_mask.shape[0]):
            if init_test_mask[i]:
                if np.argmax(y_test[i]) == np.argmax(preds[i]):
                    PreLabel[i]=1
        np.save('Mixhop_PreLable.npy',PreLabel)         
...
#[0.8909090909090909, 0.8727272727272727, 0.9454545454545454, 0.8909090909090909, 0.9636363636363636, 0.9272727272727272, 0.8909090909090909, 0.8909090909090909, 0.8909090909090909, 0.9636363636363636]
#0.9127272727272727    
